...
# build clang10
RUN git clone --single-branch --branch llvmorg-10.0.0-rc2 https://github.com/llvm/llvm-project clang10 && \
...
# build llvm10
RUN git clone --depth 1 --single-branch --branch llvmorg-10.0.0-rc2 https://github.com/llvm/llvm-project llvm && \
    cd llvm/llvm && \
    mkdir build && \
    cd build && \
